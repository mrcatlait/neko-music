...
> Amended by ADR-0014: the GraphQL/REST split still holds, but the API is no longer a published contract — Pact and `@neko/contracts` are removed and the schema is an internal build artifact.
